...
test('4: if product function are working isJSDocPropertyLikeTag', async () => {
   const user = userEvent.setup()
   render(<App />)
...
test('6: tests if Cart component is working properly', async () => {
   const user = userEvent.setup()
   render(<App />)
...
test('7: if Stripe checkout works', () => {
   const user = userEvent.setup()
   render(<App />)
